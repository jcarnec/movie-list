...
const getMovies = async (req, res) => {
  try {
    ...
    console.log(
      `db.movies.find(${JSON.stringify(filter)}).limit(100).sort(${JSON.stringify(sorting)}), db.movies.count(${JSON.stringify(countFilter)})`
    );

    // Adding explain() for query debugging
    const moviesQuery = Movie.find(filter).limit(100).sort(sorting);
    const countQuery = Movie.count(countFilter);

    // Log explanations
    console.log("Movies Query Explanation:", await moviesQuery.explain("executionStats"));
    console.log("Count Query Explanation:", await countQuery.explain("executionStats"));

    const [movies, count] = await Promise.all([moviesQuery, countQuery]);
