  ...
  start: (config, step, transferData) => {
    ...
    class Updater extends EventEmitter {
      _getLocalLatestId(done) {
        ...
        Movie.find({}, { _id: 0, id: 1 }).sort({ id: -1 }).limit(1).exec(done);
      }
      ...
      _tickDownloadedCallback(i, ticksFns, allDone, callback) {
        ...
        Logger.info(
          `Starting new tick after ${TICK_TIMEOUT / 1000} seconds...`
        );
        setTimeout(() => {
      ...
      _getTicksFunctions(ids, getDataFn) {
        ...
        for (let i = 0; i <= ticksCount; i++) {
          ...
          for (let j = rowIndex; j < rowIndex + REQUESTS_PER_TICK; j++) {
            ...
            currentTickFns.push(getDataFn.bind(this, j, ids[j], idsLength));
          }
        ...
        $this._clearNewMoviesCollection((err, result) => {
          ...
          for (let i = startId; i <= endId; i += REQUESTS_PER_TICK) {
            ...
            for (let j = i; j < i + REQUESTS_PER_TICK; j++) {
              if (j > endId) break;
              currentTickFns.push($this.getMovieDetails.bind($this, j, endId));
            }
            if (currentTickFns.length) ticksFns.push(currentTickFns);
          }

          $this._downloadPerTick(
            0,
            ticksFns,
            done,
            $this._tickDownloadedCallback.bind($this)
          );
          ...
          function done(err) {
            ...
            NewMovie.count((err, count) => {
              ...
              if (count !== expectedSavedMoviesCount) {
                Logger.warn(
                  `Expected ${expectedSavedMoviesCount} to be saved, but only ${count} were saved!`
                );
              }
      ...
      downloadMovieData(dataType, getDataFn, endEvent) {
        ...
        NewMovie.find({}, { _id: 0, id: 1 })
          ...
          .on("close", () => {
            Logger.info(
              `Found ${movieIds.length} movie ids in db. Starting to download ${dataType}...`
            );
      ...
      downloadPeople() {
        ...
        let people = [];
        NewMovie.find({}, { _id: 0, id: 1, credits: 1 })
          .cursor()
          .on("data", (movie) => {
            if (!movie.credits) return;
            if (
              movie.credits.hasOwnProperty("cast") &&
              movie.credits.cast &&
              movie.credits.cast.length
            ) {
              people = people.concat(
                movie.credits.cast.map(({ id }) => {
                  return id;
                })
              );
            }
            if (
              movie.credits.hasOwnProperty("crew") &&
              movie.credits.crew &&
              movie.credits.crew.length
            ) {
              people = people.concat(
                movie.credits.crew.map(({ id }) => {
                  return id;
                })
              );
            }
          ...
          .on("close", () => {
            Logger.info(
              `Found ${people.length} people ids in movie credits. Starting to download people...`
            );
            let alreadySavedPeople = [];
            NewPerson.find({}, { _id: 0, id: 1 })
              .cursor()
              .on("data", (person) => {
              ...
              .on("close", () => {
                Logger.info(
                  `Found ${alreadySavedPeople.length} already saved people.`
                );
                ...
                Logger.info(
                  `There are ${people.length} unsaved people. Starting to download people...`
                );
                ...
                for (let i = 0; i <= ticksCount; i++) {
                  ...
                  for (
                    let j = rowIndex;
                    j < rowIndex + REQUESTS_PER_TICK;
                    j++
                  ) {
                    if (j >= peopleIdsLength) break;
                    currentTickFns.push(
                      $this._getPerson.bind(
                        $this,
                        j,
                        people[j],
                        peopleIdsLength
      ...
      transferNewData() {
        ...
        NewMovie.find({}, { _id: 0 })
          .cursor()
          .on("data", (movie) => {
            // to avoid duplicate key error we need to delete the movie if it already exists in the movie collection
            console.log(movie);
            // here
            Movie.collection.deleteOne({ id: movie.id }, (err, result) => {
              if (err) {
                Logger.error("Error deleting existing movie:", err);
              } else {
                Movie.collection.insert(movie, (err, result) => {
                  if (err) {
                    Logger.error("Error inserting new movie:", err);
                  } else {
                    moviesCount++;
                    NewMovie.collection.deleteOne({ id: movie.id }, (err, result) => {
                        if (err) {
                            Logger.error("Error deleting new movie:", err);
                        }
                    });
          ...
          .on("close", () => {
            Logger.info(
              `${moviesCount} have been transferred from ${config.tmpMoviesCollection} to '${config.mainMoviesCollection}' collection.`
            );
            Logger.info("Starting to transfer people...");  
            let peopleCount = 0;
            NewPerson.find({}, { _id: 0 })
              .cursor()
              .on("data", (person) => {
                Person.collection.deleteOne(
                  { id: person.id },
                  (err, result) => {
                    if (err) {
                      Logger.error("Error deleting existing person:", err);
                    } else {
                      Person.collection.insert(person, (err, result) => {
                        if (err) {
                          Logger.error("Error inserting new person:", err);
                        } else {
                          peopleCount++;
                          NewPerson.collection.deleteOne({ id: person.id }, (err, result) => {
                            if (err) {
                                Logger.error("Error deleting new person:", err);
                            }
                          });
                        }
                      });
              ...
              .on("close", () => {
                Logger.info(
                  `${peopleCount} people have been transferred from '${config.tmpPeopleCollection}' to '${config.mainMoviesCollection}' collection.`
                );
    ...
    U.on(EVENTS.NEW_DATA_TRANSFERRED, (err) => {
      ...
      Logger.error(
        "All data has been transferred you can close the script now..."
      );
    });
